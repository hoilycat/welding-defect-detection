...
> Stage 2에서는 OpenCV 후보 검출, 위험도 추론, Gradio 데모와 RIAWELC→YOLO 변환·학습 파이프라인을 구현하고, 1,500장 파일럿 학습까지 완료했습니다. 대규모 본 학습과 최종 평가는 다음 단계입니다.
...
> **목표:** 검출 + 위험도 해석 + 대시보드  
> **상태:** Gradio 기반 해석 대시보드와 OpenCV 후보 검출 모드는 `main`의 `phase2/`에 구현되어 있습니다. YOLOv8 파일럿 학습에서 mAP50 0.358을 확인했으며 대규모 본 학습은 예정 단계입니다.
...
- 2026-07-21: RIAWELC 폴리곤 라벨의 YOLO 6클래스 변환기와 학습 실행기 구현, 소량 데이터 1 epoch 파이프라인 검증 완료
- 2026-07-21: Train 1,500장 / Validation 1,368장으로 YOLOv8n 10 epoch 파일럿 학습 완료 (mAP50 0.358)
- 현재 Gradio는 기본적으로 OpenCV 후보 검출 모드이며, 검증용 또는 본 학습 `best.pt` 경로를 입력하면 YOLO 추론을 우선 실행
- 다음 작업: 학습 규모 결정 → 본 학습·평가 → 최종 `best.pt` 연결

</details>

다음 단계에서는 RIAWELC 전체 또는 균형 표본 데이터로 YOLOv8 본 학습·평가를 수행하고, 최종 `best.pt` 모델을 대시보드에 연결할 예정입니다.
