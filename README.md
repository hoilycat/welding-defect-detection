...
> Stage 2에서는 RIAWELC→YOLO 변환·학습 파이프라인과 Gradio 데모를 구현하고, RT 파일럿 모델 연결까지 완료했습니다.
...
> **목표:** 검출 + 위험도 해석 + 대시보드  
> **상태:** Gradio 기반 해석 대시보드에 RT YOLOv8 파일럿 모델을 연결했습니다. 현재는 데이터 확대와 3차 학습을 준비하는 단계입니다.
...
- 다음 작업: VT 용입부족 데이터 구성 → VT 전용 모델 학습 → 앱에서 RT/VT 모델 구분

</details>

다음 단계에서는 `incomplete_penetration`(용입부족)을 포함한 VT 데이터로 전용 모델을 학습하고, 앱에서 RT/VT 모델을 구분해 사용하도록 연결합니다. 이전 6클래스 실험은 [파일럿 학습 기록](docs/yolo-pilot-training-2026-07-21.md)에 정리했습니다.
