...
> Stage 2에서는 OpenCV 후보 검출, 위험도 추론, Gradio 데모와 RIAWELC→YOLO 변환·학습 파이프라인까지 구현했으며, 전체 데이터 학습과 모델 평가는 다음 단계입니다.
...
> **목표:** 검출 + 위험도 해석 + 대시보드  
> **상태:** Gradio 기반 해석 대시보드와 OpenCV 후보 검출 모드는 `main`의 `phase2/`에 구현되어 있습니다. YOLOv8 소량 학습 검증을 완료했으며 전체 데이터 학습은 예정 단계입니다.
